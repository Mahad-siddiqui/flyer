    ...
    const processFiles = async (files: UploadedFile[]) => {
        ...
        for (const uploadedFile of files) {
            try {
                ...
                const flyerData = {
                    title: uploadResult.extractedData?.title || uploadedFile.file.name.replace(/\.[^/.]+$/, ""),
                ...
                const flyerResponse = await fetchWithTimeout('/api/flyers', {
                    method: 'POST',
                    headers: {
        ...
        <div className="container py-8 px-4 max-w-4xl">
            ...
            {session?.user?.plan?.flyerLimit !== -1 && (
                <Alert className="mb-6">
                    ...
                    <AlertDescription>
                        You have {(session?.user?.plan?.flyerLimit || 5) - (session?.user?.flyersUsed || 0)} flyers remaining in your {session?.user?.plan?.name || 'Free'} plan.
                        {((session?.user?.flyersUsed || 0) / (session?.user?.plan?.flyerLimit || 5)) > 0.8 && (
                            <span className="ml-2">
            ...
            <div className="grid gap-8">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Drag and drop your flyer files here, or click to browse. Supports JPG, PNG, GIF, and PDF files up to 10MB.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            {completedFiles.map((uploadedFile) => (
                                <div key={uploadedFile.uploadId} className="flex items-center justify-between p-4 border rounded-lg">
                                    ...
                                    <div className="flex gap-2">
                                        <Button variant="outline" asChild>
                                            <Link href={uploadedFile.flyer?.generatedUrl} target="_blank">
                                                View Page
                                            </Link>
                                        </Button>
                                        <Button asChild>
                                            <Link href={`/dashboard/flyers/${uploadedFile.flyer?.id}`}>
                                                Customize
                                            </Link>
                                        </Button>
                                    </div>
